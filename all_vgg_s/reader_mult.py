...
class Reader(object):
    ...
    def iterate(self, return_folder=False, offset=0, return_upsamp_indices=False):
        ...
        x_len = [e_idx[k] - b_idx[k] for k in range(self.n_samples)]
        ...
        queue_in = multiprocessing.Queue(self.n_samples+10)
        for k in range(self.n_samples):
            ind = index[k]
            queue_in.put((ind, b_idx[ind], e_idx[ind], upsamp_indices[ind], X_Len[ind], h_len[ind]))

        for i in range(10):
            queue_in.put(None)

        record = []
        queue_out = multiprocessing.Queue(60)
        lock = multiprocessing.Lock()  # To prevent messy print

        # input processes
        for i in range(10):
            process = multiprocessing.Process(target=self.get_feat, args=(queue_in, queue_out, lock))
            process.start()
            record.append(process)

        # main iteration
        for k in range(0, self.n_samples, self.batch_size):
            batch_size = min(self.batch_size, self.n_samples-k)
            fetched_data = [queue_out.get() for _ in range(batch_size)]
            # return indices, upsamp_indices, X_Len, h_len, feat_aug

            indices, upsamp_indices, X_Len, h_len, feat_augs = zip(*fetched_data)
            ...
            for i in range(batch_size):
                feat_aug = feat_augs[i]  	# (X_Len, 3, 101, 101)	# interp_images means interp without float
    ...
    def get_feat(self, queue_in, queue_out, lock):
        # return feat_aug
        qin_get = queue_in.get()
        while not qin_get is None:
            ind, b_id, e_id, upsamp_ind, X_Len, h_len = qin_get
            feat_raw = self.get_imgs(range(b_id, e_id))
            feat_aug = interp_images(feat_raw, upsamp_ind)
            queue_out.put([ind, upsamp_ind, X_Len, h_len, feat_aug])
            qin_get = queue_in.get()
