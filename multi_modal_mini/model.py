...
class Model(object):
    def __init__(self, phase, config, vocabulary_size=1295, hidden_ndim=512):
        ...
        # variables
        image = T.tensor4('image')  # (2*nb*len, 3, 101, 101) or (2*nb*3, 3, 101, 101)
        ...
        net['lstm_input'] = DimshuffleLayer(net['drop1d_2'], (0, 2, 1))     # (2*nb, max_hlen, 1024)

        # right hand lstm
        net['lstm_input_right'] = ExpressionLayer(net['lstm_input'], function=lambda x:x[:x.shape[0]/2], output_shape='auto')
        net['lstm_frw_right'] = LSTMLayer(incoming=net['lstm_input_right'], mask_input=net['mask'], forgetgate=Gate(b=lasagne.init.Constant(1.0)), num_units=hidden_ndim)  # (nb, max_hlen, hidden_ndim)
        net['lstm_bck_right'] = LSTMLayer(incoming=net['lstm_input_right'], mask_input=net['mask'], forgetgate=Gate(b=lasagne.init.Constant(1.0)), num_units=hidden_ndim, backwards=True)
        ...
        if phase == 'pretrain':
            ...
            # for ctc loss
            self.params_full = lasagne.layers.get_all_params(self.net['out_lin'], trainable=True)
            self.regular_params = lasagne.layers.get_all_params(self.net['out_lin'], regularizable=True)
            ...
            # full train loss
            ctc_loss_train, pred_train = self.get_ctc_loss(image, mask, token, deteministic=False)
            loss_train_full = ctc_loss_train + regular_full

            # full valid loss
            ctc_loss_valid, pred_valid = self.get_ctc_loss(image, mask, token, deteministic=True)
    ...
    def get_ctc_loss(self, image, mask, token, deteministic=False):
        ## ctc loss
        output_lin = get_output(self.net['out_lin'], {self.net['image']: image, self.net['mask']: mask}, deteministic=deteministic)
        output_softmax = Softmax(output_lin)  # (nb, max_hlen, nClasses)
