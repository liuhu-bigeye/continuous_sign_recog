...
class Model(object):
    def __init__(self, phase, config, vocabulary_size=1295, hidden_ndim=512):
        ...
        if phase == 'cnn_training':
            ...
            classify_loss_train, classify_acc1_train, classify_acc5_train = self.get_classify_loss(data, weight, mask, deterministic=False)
            classify_loss_valid, classify_acc1_valid, classify_acc5_valid = self.get_classify_loss(data, weight, mask, deterministic=True)
            ...
            self.train_func = theano.function([data, weight, mask], [loss_train_full, classify_loss_train.mean(), classify_acc1_train.mean(), classify_acc5_train.mean()], updates=updates)
            self.valid_func = theano.function([data, weight, mask], [loss_valid_full, classify_loss_valid.mean(), classify_acc1_valid.mean(), classify_acc5_valid.mean()])
    ...
    def get_classify_loss(self, data, weight, mask, deterministic=False):
        pred = get_output(self.net['classify'], {self.net['input']: data, self.net['mask']: mask}, deterministic=deterministic)
        classify_loss = - T.batched_dot(T.log(pred + self.eps), weight)

        token = weight.argmax(axis=1)
