...
database_file = '/home/liuhu/workspace/journal/all_vgg_s/output/ctc_predict_top_k_2017-04-05_20-02-09/sum_alignment_63_0.412_0.411_top_10.h5'   # weight, indices, mask
...
class Reader(object):
    def __init__(self, phase, batch_size, distortion=False, do_shuffle=False):
        ...
        # log_self(__file__)
        self.phase = phase
        self.db = h5py.File(database_file)
        # {'folder': [], 'signer': [], 'annotation': [], 'vocabulary': [], 'token': [], 'begin_index': [], 'end_index': []}
        glog.info('dataset loaded...')

        self.weights = self.db['%s_weight'%phase][...]
        self.indices = self.db['%s_indices'%phase][...]
        self.masks = self.db['%s_mask'%phase][...]
    ...
    def iterate(self):
        ...
        for k in range(0, self.n_samples, self.batch_size):
            ...
            weight = self.weights[indices].astype(np.float32)
