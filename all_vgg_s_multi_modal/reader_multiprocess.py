...
class Reader(object):
    ...
    def get_one_batch(self, queue_in, queue_out, lock):
        while not queue_in.empty():
            ...
            image = np.reshape(np.float16(image), (-1, 3, 101, 101))  # (2 * batch_size * X_len, 3, 101, 101)
            oflow = np.reshape(np.float16(oflow), (-1, 2, 101, 101)) / 20. * 128.  # (2 * batch_size * X_len, 2, 101, 101)
            coord = np.float16(coord)
