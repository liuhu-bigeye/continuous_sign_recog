...
class Model(object):
    def __init__(self, phase, config, vocabulary_size=1295, hidden_ndim=512):
        ...
        prediction_lin = T.matrix('prediction_lin')
        mask = T.tensor3('mask')
        ...
        net['classify'] = DenseLayer(ReshapeLayer(net['drop1d_2'], shape=(-1, 1024)), self.nClasses, nonlinearity=identity)
        ...
        if phase == 'cnn_training':
            ...
            loss_hard_train, loss_soft_train, classify_acc1_train, classify_acc5_train = self.get_classify_loss(data, prediction_lin, mask, deterministic=False)
            loss_hard_valid, loss_soft_valid, classify_acc1_valid, classify_acc5_valid = self.get_classify_loss(data, prediction_lin, mask, deterministic=True)

            loss_train_full = regular_full + loss_hard_train.mean() * 0.1 + loss_soft_train.mean() * 0.9
            loss_valid_full = regular_full + loss_hard_valid.mean() * 0.1 + loss_soft_valid.mean() * 0.9

            updates = lasagne.updates.adam(loss_train_full, self.params_full, learning_rate=self.learning_rate)
            self.train_func = theano.function([data, prediction_lin, mask], [loss_train_full, loss_hard_train.mean(), loss_soft_train.mean(), classify_acc1_train.mean(), classify_acc5_train.mean()], updates=updates)
            self.valid_func = theano.function([data, prediction_lin, mask], [loss_valid_full, loss_hard_valid.mean(), loss_soft_valid.mean(), classify_acc1_valid.mean(), classify_acc5_valid.mean()])
    ...
    def get_classify_loss(self, data, prediction_lin, mask, deterministic=False):
        pred_lin = get_output(self.net['classify'], {self.net['input']: data, self.net['mask']: mask}, deterministic=deterministic)
        ...
        loss_hard = lasagne.objectives.categorical_crossentropy(pred_hard, token)
        loss_soft = - T.sum(prediction_soft * T.log(pred_soft + 1e-20), axis=1)

        classify_acc_1 = lasagne.objectives.categorical_accuracy(pred_hard, token, top_k=1)
        classify_acc_5 = lasagne.objectives.categorical_accuracy(pred_hard, token, top_k=5)
