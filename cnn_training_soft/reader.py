...
database_file = '/home/liuhu/workspace/journal/all_vgg_s/output/ctc_predict_top_k_2017-04-05_20-02-09/soft_alignment_63_0.412_0.411_top_10.h5'   # token, weight, indices, mask
...
class Reader(object):
    def __init__(self, phase, batch_size, distortion=False, do_shuffle=False):
        ...
        # log_self(__file__)
        self.phase = phase
        self.df = h5py.File(database_file)
            # {'folder': [], 'signer': [], 'annotation': [], 'vocabulary': [], 'token': [], 'begin_index': [], 'end_index': []}
        glog.info('dataset loaded...')

        self.prediction_lin = self.df['%s_prediction_lin'%phase][...]
        self.indices = self.df['%s_indices'%phase][...]
        self.masks = self.df['%s_mask'%phase][...]
        ...
        self.n_samples = self.masks.shape[0]
    ...
    def iterate(self):
        ...
        for k in range(0, self.n_samples, self.batch_size):
            ...
            img_indices = [[ii for ii in self.indices[i] if ii != -1] for i in indices]
            ...
            mask_pre = map(lambda x:list(x).index(1), mask)
            ...
            prediction_lin = self.prediction_lin[indices].astype('float32')

            yields = [feat, prediction_lin, mask]
